...
**Delivered in Phase 6.** Every governed tool dispatch now freezes an Effect
Record before execution: operation/argument digest, harness-owned side-effect
class, replay policy, idempotency key, authority snapshot, attempts, and
observations. Fixed file writes demonstrate the complete protocol across direct
timeouts and process restart: independently observed application continues once,
proven non-application permits bounded same-key replay, and unknown outcomes
...
**Delivered in Phase 7B1.1.** A frozen same-endpoint comparison now exercises
TaiYi, pinned Pi 0.84.1, and published OpenClaw 2026.8.1-beta.1 through real
streaming HTTP, one tool action, terminal-envelope parsing, and an evaluator
outside every harness. External adapters use isolated home/config/state, no
ambient credentials or extensions, a macOS deny-by-default wrapper, and canaries
for workspace confinement and exact-port network access. TaiYi additionally
enforces the same capability allowlist in its model hint and executor. Observed
model requests must expose exactly the frozen read/write tool surface or the cell
is automatically excluded. All three comparable cells passed repeated runs with
two model requests, one tool call, independent artifact delivery, and zero false
completions. ZCode remains explicit `NOT_COMPARABLE` because its required batch
evidence interface is unavailable. This layer tests adapter transport/tool truth,
not model intelligence or harness ranking. See
`learning/docs/12_Controlled_Cross_Harness_Comparison.md`,
...
**Delivered in Phase 7B2.4.** Repository refresh now runs as an idempotent
background supervisor job with an immutable request and typed result receipt.
New tasks coalesce on a live repository generation but advance after terminal
settlement; recovered tasks retain their checkpointed generation and reattach to
the same job id. Gateway status and cancellation understand repository jobs
separately from tool jobs. Subscriber-level cancellation terminates a sole job
but does not break another task sharing the index. Fault tests cover concurrent
coalescing, sequential freshness, cancellation isolation, startup failure, and
Gateway-owner exit after attachment. A production run against the pinned
25,683-file Kubernetes checkout completed the first generation in 7.190 seconds
...
**Delivered in Phase 7B2.6.** Every durable supervisor now appends an fsync'd
terminal wake notification after its authoritative result. The repository wake
loop consumes that journal incrementally by byte cursor and retains a
lease-bound JobRecord fallback instead of scanning every checkpoint and job at
50 ms intervals. RunStore supports immediate local waits and bounded
cross-process refresh; clients can use JSON long polling or revision-addressed
SSE with `Last-Event-ID`. Tests cover HTTP streaming, reconnect replay exclusion,
partial/corrupt notifications, missing-hint fallback, cancellation, and restart.
A pinned Kubernetes run deliberately disconnected at revision 4, resumed at 5
without replay, and settled at 12 from one 7.139-second job. See
`learning/docs/19_Durable_Event_Notifications_and_SSE.md`.

**Delivered in Phase 7B2.7.** Asynchronous Agent and Workflow shell tools now
park only after the stable operation, Effect Record dispatch, durable job id,
and frozen continuation are checkpointed. The original task thread and task
lease are released while the supervisor runs. One generic Gateway wake loop
incrementally consumes repository and tool journals, falls back to authoritative
JobRecord polling when a hint is lost, and schedules recovery only after a
terminal receipt. Restart, cancellation, missing-notification, both-runtime,
same-job, and no-replay tests preserve the existing Effect Ledger boundary. A
pinned Kubernetes run read 31,297 files and 277.6 MB, parked in 0.023 seconds,
survived Gateway replacement, and settled from one 2.147-second Job with one
